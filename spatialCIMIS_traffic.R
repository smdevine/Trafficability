...
dim(ETo)
totalData <- 5479*(24566-5) #134,569,719 records
sum(is.na(ETo[,6:ncol(ETo)])) #36,043 NAs; this is 0.03% missing
sum(!is.na(ETo$cell_190395)) #only 71 days with data; remove
ETo$cell_190395 <- NULL
sum(ETo[,6:ncol(ETo)] < 0, na.rm = TRUE) #2,153 negative numbers
sum(ETo[,6:ncol(ETo)] == 0, na.rm = TRUE) #7474 observations equal to 0
...
neg_presence_ETo
length(neg_presence_ETo) #1559 cells with negative ETo values
...
sum(ETo[,6:ncol(ETo)] == 0, na.rm = TRUE) #still have 7474
...
zero_presence_ETo
length(zero_presence_ETo) #124 cells with ETo values of zero 1 day
