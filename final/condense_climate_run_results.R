...
traffDefs <- read.csv(file.path(dataDir, 'traffic_defs_by_texture5.7.21.csv'), stringsAsFactors = FALSE) #revised definitions to 0.85-0.95, scaled by typical plasticity index by textural class
...
colnames(soil_df)
traffDefs[,1:2]
all(soil_df$textural_class %in% traffDefs$textural.classes)
soil_df$traff_def_final <- traffDefs$traffic.def[match(soil_df$textural_class, traffDefs$textural.classes)]
head(soil_df[,c('textural_class', 'traff_def_final')], 20)
...
for (i in 1:length(subDirs)) {
  ...
  results_template <- soil_df[,c('cokey', 'textural_class', 'traff_def_final')]
  ...
  for (j in 1:length(subDirs2)) {
    ...
    if(length(cokeys_modeled)!=2911){
      ...
      if(length(trafficability)!=2911) {
        ...
        for (k in 1:length(errors_soils_final)) {
