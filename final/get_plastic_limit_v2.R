...
#after original query, start here
SSURGO_horizons <- read.csv(file.path(soilsDir, 'SSURGO_horizons_data.csv'), stringsAsFactors = FALSE)
...
#limit to those soils included in modeling work
soils_df <- read.csv(file.path(tablesDir, 'reduce_soils', 'soils_modeled_revised_QCpass_Oct2020.csv'), stringsAsFactors = FALSE)
SSURGO_horizons_modeled_test <- SSURGO_horizons_modeled[SSURGO_horizons_modeled$cokey %in% soils_df$cokey,]
dim(SSURGO_horizons_modeled_test) #reduced from 11,466 to 5,764
# SSURGO_horizons_modeled_test <- SSURGO_horizons_modeled
lapply(textures, function(x) hist(SSURGO_horizons_modeled_test$lpl_to_theta_fc[SSURGO_horizons_modeled_test$textural_class==x], main=x, xlab='PL:FC'))
lapply(textures, function(x) hist(SSURGO_horizons_modeled_test$pi_r[SSURGO_horizons_modeled_test$textural_class==x], main=x, xlab='plasticity index'))
lapply(textures, function(x) median(SSURGO_horizons_modeled_test$pi_r[SSURGO_horizons_modeled_test$textural_class==x]))
lapply(textures, function(x) mean(SSURGO_horizons_modeled_test$pi_r[SSURGO_horizons_modeled_test$textural_class==x]))


pi_by_texture <- as.data.frame(aggregate(SSURGO_horizons_modeled_test$pi_r, list(SSURGO_horizons_modeled_test$textural_class), summary))
pi_by_texture
pi_by_texture$n <- tapply(SSURGO_horizons_modeled_test$pi_r, SSURGO_horizons_modeled_test$textural_class, function(x) sum(!is.na(x)))
pi_by_texture$sd <- tapply(SSURGO_horizons_modeled_test$pi_r, SSURGO_horizons_modeled_test$textural_class, sd)
pi_by_texture
write.csv(pi_by_texture, file.path(tablesDir, 'plasticity_index_by_texture.csv'), row.names = TRUE)
