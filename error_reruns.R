...
# cellnames
# date_of_interest <- '2015-01-15'

#error file to select
error_files <- list.files(path = file.path(modelDir, 'errors'))
error_files <- error_files[grepl(glob2rx('*04-15*'), error_files)]
...
for (j in 1:length(error_files)) { #length(error_files)
  options(max.print = 15000)
  options(width=130)
  error_file <- read.csv(file.path(modelDir, 'errors', error_files[j]), stringsAsFactors = FALSE)
  error_soils <- error_file$soilnames
  error_soils_data <- soil_data[names(soil_data) %in% error_soils]
  cellname <- paste0('cell_', unlist(strsplit(error_files[j], '_'))[4])
  date_of_interest <- sub('.csv', '', unlist(strsplit(error_files[j], '_'))[5])
  for(i in 1:length(error_soils_data)) {
    write_atmos(modelDir = modelDir, flood_date = date_of_interest, flood_duration = 4, soil_name = names(error_soils_data)[i], PET = ETo_cells_of_interest, CIMIS=cellname)
    write_selector(mat_number = error_soils_data[[i]]$mat_number, flood_duration = 4, modelDir = modelDir, VGs = error_soils_data[[i]]$VGs, soil_name = names(error_soils_data)[i], MaxIT = 90, CIMIS = cellname, flood_date = date_of_interest)
    write_profile(depths=error_soils_data[[i]]$depths, soil = error_soils_data[[i]]$soil, mat_number = error_soils_data[[i]]$mat_number, modelDir = modelDir, soil_name = names(error_soils_data)[i], CIMIS = cellname, flood_date = date_of_interest)
  }
  #write file paths and run.bat file
  paths_shortcut <- paste0('path', 1:length(error_soils_data))
  ...
  for (i in seq_along(paths_shortcut)) {
    ...
    writeLines(file.path('D:/PostDoc/Trafficability/climate_runs', sub_Dir, names(error_soils_data)[i], names(error_soils_data)[i], fsep='/'), con = filepth)
    close(filepth)
  }
  print(options()$width)
}
